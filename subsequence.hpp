...
bool is_nonincreasing(const sequence& A) {
  ...
  //check each adjacent pair in A-array starting at index 0
  for (int i = 0; i < A.size()-2; i++) {
    if (A[i] < A[i+1]) { //check if right element is greater than left element
      return false; //return false, NOT non-increasing sequence
    }
  }
...
sequence longest_nonincreasing_end_to_beginning(const sequence& A) {
  ...
  for (signed int i = n-2;  i>= 0; i--) {
    for (size_t j = i+1; j < n ; j++) {
      ...
      //check adjacent pairs in A-array then adjacent corresponding pairs in H-array
      if (A[j] <= A[i] && H[j] >= H[i]) {
        H[i] = H[j] + 1; //if both conditions pass, current index i has smaller element trailing
      }
    }
  }
  ...
  for (size_t i = 0; i < n; ++i) {
    if (H[i] == index) {
      R[j] = A[i]; //if H-value is index's value, add A-array elemnt to R
      j++; //update j
      index--; //update index position
...
sequence longest_nonincreasing_powerset(const sequence& A) {
  ...
  while (true) {
    ...
    //if candidate passes non-increasing test and it's size is bigger than best's
    if (is_nonincreasing(candidate) == true && candidate.size() > best.size()) {
      best = candidate; //update best with sequenced elements in candidate
    }
